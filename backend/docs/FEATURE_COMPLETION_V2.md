# AIHub Governance Enhancement v2
...
- `dbapikey.Provider` 会从 `aihub_token` 表按 token hash 查询 active token。
...
- 如果 principal 没有显式 namespace 权限，则查询 `aihub_namespace_member`。
- namespace 成员角色可授权：
  - `owner/admin`：管理权限。
